...
- `4 執行`：選擇是否監看資料夾，並開始或停止分析；結果會直接顯示在右側表格。500 張是資料夾容量上限，不是進度條分母；進度條會以目前資料夾已偵測到的照片數計算，全部辨識完成就會顯示 100%。若只想處理目前資料夾內照片，請取消勾選 `監看資料夾`；監看模式會在目前照片處理完後繼續等待新照片。
...
若 AI 辨識錯誤，可在縮圖下方按 `修正重辨`，輸入正確資訊，例如「這張是鹹蛋苦瓜，不是炒高麗菜」。程式會把原列標成待移除，並在最後新增一筆待修正任務；目前 AI 工作完成後會用同一張原圖與修正資訊重新送 API，成功後刪掉原本錯誤列，只留下正確列，不會多佔 500 張名額。
...
若要換下一批照片，請先按 `停止`，更換或清空照片資料夾後按 `新一批` 初始化畫面與進度，再按 `開始`。
